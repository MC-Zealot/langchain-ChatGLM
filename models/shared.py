...
def loaderLLM_and_chain(llm_model: str = None, no_remote_model: bool = False, use_ptuning_v2: bool = False) -> Any:
    """
    init llm_model_ins LLM
    :param llm_model: model_name
    :param no_remote_model:  remote in the model on loader checkpoint, if your load local model to add the ` --no-remote-model
    ...
    llm = loaderCheckPoint.model
    provides_class = getattr(sys.modules['models'], llm_model_info['provides'])
    modelInsLLMChain = provides_class(checkPoint=loaderCheckPoint)
    if 'FastChatOpenAILLM' in llm_model_info["provides"]:
        modelInsLLMChain.set_api_base_url(llm_model_info['api_base_url'])
        modelInsLLMChain.call_model_name(llm_model_info['name'])
        modelInsLLMChain.set_api_key(llm_model_info['api_key'])
    return modelInsLLMChain, llm