...
embedding_model_dict = {
    ...
    "text2vec-base": embedding_model_path + "/text2vec-base-chinese",
    "text2vec": embedding_model_path + "/text2vec-large-chinese",
...
PROMPT_TEMPLATE_V2 = """已知信息：
{context} 
计算公式如下：
速动比率=(流动资产-存货)/流动负债
毛利率=(营业收入-营业成本)/营业收入
企业研发费用占费用，这里的费用是指销售费用+财务费用+管理费用+研发费用
三费比重指占的营业收入的比重
营业成本率= 营业成本/营业收入
研发经费与利润比例中的利润是净利润
企业研发经费与利润比值=研发费用/净利润

结合上述已知信息，简洁和专业的来回答用户的问题，回答字数限制在1000个字以内。如果无法从中得到答案，请说 “根据已知信息无法回答该问题”，不允许在答案中添加编造成分，答案请使用中文。 问题是：{question}"""
