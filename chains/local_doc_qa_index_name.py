...
def generate_prompt_v2(related_docs: List[str],
    ...
    context = "\n".join([doc.page_content for doc in related_docs])
    context=context[0:2650].replace(' ','')#chatglm6b最大的prompt是8898，所以不能超了字数，做截断
    prompt = prompt_template.replace("{question}", query).replace("{context}", context)
    return prompt
...
class LocalDocQA:
    ...
    def get_prompt_based_query(self, query, vs_path, index_name,ori_query):
        ...
        torch_gc()
        if len(related_docs_with_score) > 0:
            prompt = generate_prompt_v2(related_docs_with_score, ori_query)
        else:
            prompt = ori_query
